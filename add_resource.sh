...
# Usage: 
# For RSS/Scraping: ./add_resource.sh <private_key> rss <url> <weight>
# For Quotes:       ./add_resource.sh <private_key> quote <categories> <weight>

PRIVATE_KEY=$1
TYPE=$2
# $3 is either URL or Categories
PARAM_3=$3
WEIGHT=$4

if [ -z "$PRIVATE_KEY" ] || [ -z "$TYPE" ] || [ -z "$PARAM_3" ] || [ -z "$WEIGHT" ]; then
  echo "Usage:"
  echo "  For RSS/Scraping: ./add_resource.sh <private_key> <type> <url> <weight>"
  echo "  For Quotes:       ./add_resource.sh <private_key> quote 'cat1,cat2,cat3' <weight>"
  echo "Example (RSS): ./add_resource.sh nsec1... rss https://example.com/rss.xml 5"
  echo "Example (Quote): ./add_resource.sh nsec1... quote 'technology,science' 3"
  exit 1
fi
...
# Create the new resource object based on type
if [ "$TYPE" == "rss" ] || [ "$TYPE" == "scraping" ]; then
  NEW_RESOURCE=$(jq -n \
                    --arg type "$TYPE" \
                    --arg url "$PARAM_3" \
  ...
  # Split comma-separated string into a JSON array of strings
  CATEGORIES_JSON=$(echo "$PARAM_3" | jq -R 'split(",")' | jq -c '.')
  NEW_RESOURCE=$(jq -n \
                    --arg type "$TYPE" \
                    --argjson categories "$CATEGORIES_JSON" \
                    --argjson weight "$WEIGHT" \
                    '{type: $type, categories: $categories, weight: $weight}')
else
    echo "Invalid resource type: $TYPE. Must be 'rss', 'scraping', or 'quote'."
    exit 1
fi
